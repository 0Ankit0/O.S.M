...
class TenantManager(models.Manager):
    def get_or_create_user_default_tenant(self, user):
        ...
        """
        default_tenant = self.filter(created_by=user, type=TenantType.DEFAULT).order_by("created_at").first()
        if default_tenant:
            return default_tenant, False

        new_tenant = self.create(created_by=user, type=TenantType.DEFAULT, name=str(user))
