...
@event_handlers.djstripe_receiver("subscription_schedule.canceled")
def activate_free_plan_on_subscription_deletion(sender, event: djstripe_models.Event, **kwargs):
    """
...
@event_handlers.djstripe_receiver("subscription_schedule.released")
def capture_release_schedule(sender, event: djstripe_models.Event, **kwargs):
    """
...
@event_handlers.djstripe_receiver("payment_method.attached")
def update_subscription_default_payment_method(sender, event: djstripe_models.Event, **kwargs):
    """
...
@event_handlers.djstripe_receiver("payment_method.detached")
def remove_detached_payment_method(sender, event: djstripe_models.Event, **kwargs):
    obj = event.data["object"]
    djstripe_models.PaymentMethod.objects.filter(id=obj["id"]).delete()


@event_handlers.djstripe_receiver(["invoice.payment_failed", "invoice.payment_action_required"])
def cancel_trial_subscription_on_payment_failure(sender, event: djstripe_models.Event, **kwargs):
    obj = event.data["object"]
...
@event_handlers.djstripe_receiver(["invoice.payment_failed", "invoice.payment_action_required"])
def send_email_on_subscription_payment_failure(sender, event: djstripe_models.Event, **kwargs):
    """
...
@event_handlers.djstripe_receiver("customer.subscription.trial_will_end")
def send_email_trial_expires_soon(sender, event: djstripe_models.Event, **kwargs):
    obj = event.data["object"]
...
@event_handlers.djstripe_receiver("charge.refund.updated")
def charge_refund_updated(sender, event: djstripe_models.Event, **kwargs):
    """
